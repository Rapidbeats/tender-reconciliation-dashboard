...
class TenderReconciliationProcessor:
    ...
    def _safe_get(self, series_like, key, default=0):
        """Safely get value from a pandas Series or mapping; return default if missing."""
        try:
            # If it's a pandas Series
            if hasattr(series_like, 'get') and not hasattr(series_like, 'iloc'):
                return series_like.get(key, default)

            # For Series or DataFrame index lookup
            if hasattr(series_like, 'loc'):
                try:
        ...
        for store_id in exception_store_ids:
            ...
            if not exceptional_items.empty:
                ...
                if abs(new_total) >= 100:
                    ...
                    total_auto = self._safe_get(total_auto_updated_responses, store_id, 0)
                    total_auto_for_rate = total_auto if total_auto > 0 else 1

                    summary_row = {
                        'Store_ID': store_id,
                        'Total_Auto-Updated_Responses': total_auto,
                        ...
                        'Error_Rate_%': (line_item_count / total_auto_for_rate * 100) if total_auto > 0 else 0
                    }
